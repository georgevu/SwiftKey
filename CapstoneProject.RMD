...
createSampleFile <- function(infile, outfile, num_samples) {
  ...
  con <- file(outfile, "w") 
  writeLines(lines, con, encoding = "UTF-8")
  close(con)
  lines
}
readSampleFile <- function() {
  ...
  writeLines(trainingData, out, encoding = "UTF-8")
  close(out)
  out <- file(testingDataFile, "w") 
  writeLines(testingData, out, encoding = "UTF-8")
  close(out)
  return(trainingData)
}
...
createDataFiles <- function() {
  ...
  writeLines(lines, outSample, encoding = "UTF-8") 
...
predict <- function(text, numSelections) {
  ...
  for (i in min(n-1, 4):1) {
    if (i == 4) {
      ...
      selected <- gramTable5[text %like% searchText]
    } else if (i == 3) {
      searchText <- trimws(paste(textTokens[n-i+1], textTokens[n-i+2], textTokens[n-i+3], collpase = ""))
      searchText <- paste0("^", searchText, " ", collapse="")
      selected <- gramTable4[text %like% searchText]
    } else if (i == 2) {
      searchText <- trimws(paste(textTokens[n-i+1], textTokens[n-i+2], collapse = ""))
      searchText <- paste0("^", searchText, " ", collapse="")
      selected <- gramTable3[text %like% searchText]
    } else {
      searchText <- paste0("^", trimws(textTokens[n-i+1]), " ", collapse="")
      selected <- gramTable2[text %like% searchText ]
    } 
    if (nrow(selected) > 0) {
...
createNGrams <- function(ngram) {
  ...
  grams <- tokens_tolower(grams)
  dfm <- dfm(grams)
  freq <- docfreq(dfm)
  df <- as.data.frame(freq)
  dt <- data.table(text=row.names(df), frequency=freq)
  setkey(dt, "text")
  dt <- dt[order(text)]
  return (dt)
}
