...
predict <- function(text, numSelections) {
  ...
  for (i in min(n-1, 4):1) {
    if (i == 4) {
      ...
      selected <- gramTable5["gram", on=searchText, order("frequency")]
    } else if (i == 3) {
      searchText <- trimws(paste(textTokens[n-i+1], textTokens[n-i+2], textTokens[n-i+3], collpase = ""))
      selected <- gramTable4["gram", on=searchText, order("frequency")]
    } else if (i == 2) {
      searchText <- trimws(paste(textTokens[n-i+1], textTokens[n-i+2], collapse = ""))
      selected <- gramTable3["gram", on=searchText, order("frequency")]
    } else {
      selected <- gramTable2["gram", on=searchText, order("frequency")]
    } 
    if (nrow(selected) > 0) {
...
createNGrams <- function(ngram) {
  ...
  dfm <- dfm(grams)
  freq <- docfreq(dfm, scheme="count")
  df <- as.data.frame(freq)
  df$text <- rownames(df)
  getKey <- function(x) {
    if (ngram == 2) {
      ul = paste(word(x, ngram-1), collapse = "")
    } else {
  ...
  dt <- data.table(gram=lapply(df[,2], getKey), 
                   prediction=lapply(df[,2], getToken),
                   frequency=df[,1])
  setkey(dt, "gram")
  #dt <- dt[order(gram)]
  return (dt)
